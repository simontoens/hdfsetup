...
class StartupSequenceBuilder:
    ...
    def _stamp(self):
        ...
        self.comment("Generated by hdfsetup on %s/%s/%s" % (now.month, now.day, now.year))
...
if __name__ == "__main__":
    ...
    if os.path.exists(cdir):
        ...
        os.mkdir(root_cdir)
        copy_tree(cdir, root_cdir)

    sb = StartupSequenceBuilder()
    sb.cd(unpack_root_name)
    sb.comment("start of original startup-sequence")
    with open(os.path.join(unpack_root, "s", "startup-sequence")) as f:
        sb.addContent(f.read())
